RegisterServerEvent("bcc-society:RankManagement", function(type, businessId, rankName, rankLabel, rankPay, payIncrement, toggleBlip, withdraw, deposit, editRanks, manageEmployees, openInventory, editWebhook, canManageStore, rankJobGrade, rankCanBillPlayers, rankCanSwitchJob)
    ...
    if rankCanSwitchJob ~= "true" then
        rankCanSwitchJob = "false"
    end
    ...
    local permissionsList = "**Permissions**:\n" ..
        ...
        "- " .. _U("canSwitchJob") .. ": " .. (rankCanSwitchJob == "true" and "✅" or "❌")
    ...
    elseif type == "update" then
        ...
        if #retval > 0 then
            ...
            MySQL.query.await("UPDATE bcc_society_ranks SET rank_label = ?, rank_pay = ?, rank_pay_increment = ?, rank_can_toggle_blip = ?, rank_can_withdraw = ?, rank_can_deposit = ?, rank_can_edit_ranks = ?, rank_can_manage_employees = ?, rank_can_open_inventory = ?, rank_can_edit_webhook_link = ?, rank_can_manage_store = ?, society_job_rank = ?, rank_can_bill_players = ?, rank_can_switch_job = ? WHERE business_id = ? AND rank_name = ?",
                { rankLabel, rankPay, payIncrement, toggleBlip, withdraw, deposit, editRanks, manageEmployees, openInventory, editWebhook, canManageStore, rankJobGrade, rankCanBillPlayers, rankCanSwitchJob, businessId, rankName })
