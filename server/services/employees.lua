BccUtils.RPC:Register("bcc-society:GetAllEmployeesData", function(params, cb, recSource)
    ...
    if employees and type(employees) == "table" and #employees > 0 then
        ...
        for k, v in pairs(employees) do
            table.insert(employeesCharIds,
                { charId = v.employee_id, employeeName = v.employee_name, employeeRank = v.employee_rank })
        end
...
RegisterServerEvent('bcc-society:PlayerHired', function(businessId, playerCharId, playerSource, societyData)
    ...
    if not Config.employeeWorksAtMultiple then
        ...
        "SELECT * FROM bcc_society_employees WHERE employee_id = ? AND business_id = ?", { playerCharId, businessId })
        ...
        if #employee > 0 or #owner > 0 then
            ...
            "SELECT rank_name, society_job_rank FROM bcc_society_ranks WHERE business_id = ? ORDER BY society_job_rank ASC LIMIT 1",
                { businessId })
            ...
            if rankInfo and #rankInfo > 0 then
                ...
                "INSERT INTO bcc_society_employees (employee_id, business_id, employee_name, employee_rank) VALUES (?, ?, ?, ?)",
                    { playerCharId, businessId, employeeFullName, rankInfo[1].rank_name })
...
RegisterServerEvent('bcc-society:ChangeEmployeeRank', function(businessId, rankName, employeeId, businessName)
    local _source = source
    local retval = MySQL.query.await("SELECT * FROM bcc_society_employees WHERE business_id = ? AND employee_id = ?",
        { businessId, employeeId })

    if #retval > 0 then
        -- Retrieve the rank data to get society_job_rank associated with rankName
        local rankData = MySQL.query.await(
        "SELECT society_job_rank FROM bcc_society_ranks WHERE business_id = ? AND rank_name = ?",
            { businessId, rankName })
        ...
        if rankData and #rankData > 0 then
            ...
            "UPDATE bcc_society_employees SET employee_rank = ? WHERE business_id = ? AND employee_id = ?",
                { rankName, businessId, employeeId })
...
BccUtils.RPC:Register("bcc-society:GetPaymentAmount", function(params, cb, recSource)
    ...
    local employeeData = MySQL.query.await("SELECT employee_payment FROM bcc_society_employees WHERE employee_id = ?",
        { char.charIdentifier })
...
RegisterServerEvent("bcc-society:CollectPayment", function()
    ...
    local employeeData = MySQL.query.await(
    "SELECT e.employee_payment, s.ledger, s.webhook_link, s.business_id FROM bcc_society_employees e JOIN bcc_society s ON e.business_id = s.business_id WHERE e.employee_id = ?",
        { char.charIdentifier })
    if employeeData and #employeeData > 0 then
        ...
        if payAmount and payAmount > 0 then
            if ledgerAmount >= payAmount then
                ...
                local ledgerUpdate = MySQL.query.await(
                "UPDATE bcc_society SET ledger = ledger - ? WHERE business_id = ?", { payAmount, societyId })
                -- Confirm the updated ledger amount
                local updatedLedgerData = MySQL.query.await("SELECT ledger FROM bcc_society WHERE business_id = ?",
                    { societyId })
                ...
                BccUtils.Discord.sendMessage(employeeData[1].webhook_link, Config.WebhookTitle, Config.WebhookAvatar,
                    "Payment Collected", paymentMessage)
...
RegisterServerEvent("bcc-society:PayEmployee", function(payAmount, societyId)
    ...
    local employeeData = MySQL.query.await(
    "SELECT employee_payment FROM bcc_society_employees WHERE employee_id = ? AND business_id = ?",
        { char.charIdentifier, societyId })
    if employeeData and #employeeData > 0 then
